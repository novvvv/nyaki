...
**형제 카드도 한 세션에 같이 낸다.** 한때 안키의 bury siblings를 흉내 내어 같은
출처의 카드를 한 장만 냈지만, 빈칸을 두 개 만들어놓고 한 개만 나오는 것이
"카드 2장"이라는 화면 표시와 어긋났다. 안키도 기본값은 묻지 않는 것이다 —
c1의 앞면이 c2의 답을 보여주는 것은 사용자가 감수하는 쪽을 택했다.
...
**웹이 SM-2를 계산하지 않는 구조는 유지된다.** 채점 이벤트는 세션 끝에 모아 보내고
서버가 받은 순서대로 적용한다.

**세션 안에서는 같은 카드를 다시 내지 않는다**(2026-09-24). 학습 단계가 잡아둔
다음 시각(1분·10분 뒤)이 지나면 **다음 테스트**에서 나온다. 세션 안에서 되돌리면
